...
export default function Sidebar({ mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const showGettingStarted = isOwnerOrAdmin && onboardingProgress && onboardingProgress.total > 0 && onboardingProgress.completed < onboardingProgress.total;

  const sections: NavSection[] = [
    // Getting Started - only while onboarding incomplete
    ...(showGettingStarted ? [{
      label: '',
      items: [{
    ...
    // FOLLOW-UP - hidden for evaluators
    ...(!isEvaluator ? [{
      label: 'FOLLOW-UP',
      items: [
        {
    ...
    // INSIGHTS - hidden for evaluators
    ...(!isEvaluator ? [{
      label: 'INSIGHTS',
      items: [
        {
